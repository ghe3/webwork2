...
if(defined($filename) && $filename =~ m/^\w+.{1}p{1}m{1}/){
	my $shortname = substr($filename, 0, index($filename, "."));
	my $plfilename = $shortname."_to_po.pl";

	open(PLFILE, ">".$plfilename) || die "Can't open!";
	my $contents = <<CONTENT;
#!/usr/bin/perl -w 
...
open(POFILE, ">$shortname.po") || die "Can't open!";

\%lex =  \%WeBWorK::Localize::${shortname}::Lexicon;
...
CONTENT
	...
	print PLFILE $contents;
	
	close(PLFILE) || die "Can't close!";
	
	system("perl", $plfilename);
}
else{
